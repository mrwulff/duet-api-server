...
function _processTypeformV() {_processTypeformV = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(req, res) {var answers, formTitle, formQuestions, eventId, language, beneficiaryId, phoneNum, photoUrl, itemName, price, size, store, itemTranslationResult, itemNameEnglish, categoryId, storeId, itemId, _sqlHelpers$insertIte, code, s3PhotoUrl;return regeneratorRuntime.wrap(function _callee$(_context) {while (1) {switch (_context.prev = _context.next) {case 0:_context.prev = 0;console.log("Processing TypeForm (V4)");answers = req.body.form_response.answers;formTitle = req.body.form_response.definition.title;formQuestions = req.body.form_response.definition.fields;eventId = req.body.form_response.eventId; // Check which language was used
            ...
            if (!(answers.length >= 8)) {_context.next = 60;break;}beneficiaryId = answers[0].text;phoneNum = answers[1].phone_number;photoUrl = encodeURI(answers[2].file_url);itemName = answers[4].choice.label; // replace "," with "."; remove non-numeric characters
            price = answers[5].text.replace(/,/g, '.').replace(":", ".").replace(/[^\d.]/g, '');size = null;store = null;if (!(answers.length == 8)) {_context.next = 20;break;}store = answers[6].choice.label;_context.next = 26;break;case 20:if (!(answers.length == 9)) {_context.next = 25;break;}size = answers[6].text;store = answers[7].choice.label;_context.next = 26;break;case 25:throw "Invalid number of answers: " + answers.length;case 26:if (!(isNaN(price) || price <= 0)) {_context.next = 28;break;}throw "Invalid price: " + answers[5].text;case 28:_context.next = 30;return _sqlHelpers["default"].getItemNameTranslation(language, itemName);case 30:itemTranslationResult = _context.sent;if (!itemTranslationResult) {console.log("Invalid item name! Table: name_" + language + "; itemName: " + itemName);res.status(500).send();}itemNameEnglish = itemTranslationResult.name_english;categoryId = itemTranslationResult.category_id;_context.next = 36;return _sqlHelpers["default"].getStoreIdFromName(store);case 36:storeId = _context.sent;_context.prev = 37;_context.next = 40;return _sqlHelpers["default"].insertItemFromTypeform((_sqlHelpers$insertIte = { itemNameEnglish: itemNameEnglish, size: size, price: price, beneficiaryId: beneficiaryId }, _defineProperty(_sqlHelpers$insertIte, "beneficiaryId", beneficiaryId), _defineProperty(_sqlHelpers$insertIte, "categoryId", categoryId), _defineProperty(_sqlHelpers$insertIte, "storeId", storeId), _defineProperty(_sqlHelpers$insertIte, "photoUrl", photoUrl), _defineProperty(_sqlHelpers$insertIte, "in_notification", 1), _sqlHelpers$insertIte));case 40:itemId = _context.sent;_context.next = 47;break;case 43:_context.prev = 43;_context.t0 = _context["catch"](37); // Sendgrid Error message (email)
            _sendgridHelpers["default"].sendTypeformErrorEmail({ formTitle: formTitle, eventId: eventId, err: _context.t0 });res.status(500).send();case 47: // get code for item
            code = _itemHelpers["default"].generatePickupCode(itemId);_context.next = 50;return _sqlHelpers["default"].updateItemPickupCode(itemId, code);case 50:_context.next = 52;return _s3Helpers["default"].uploadItemImageToS3(itemId, photoUrl);case 52:s3PhotoUrl = _context.sent;_context.next = 55;return _sqlHelpers["default"].updateItemPhotoLink(itemId, s3PhotoUrl);case 55:_context.next = 57;return _sqlHelpers["default"].setSingleStoreNotificationFlag(storeId);case 57:res.status(200).send();_context.next = 62;break;case 60:console.log("Error! Invalid number of answers.");res.status(502).send();case 62:_context.next = 68;break;case 64:_context.prev = 64;_context.t1 = _context["catch"](0);_errorHandler["default"].handleError(_context.t1, "typeform/processTypeformV4");res.status(500).send();case 68:case "end":return _context.stop();}}}, _callee, null, [[0, 64], [37, 43]]);}));return _processTypeformV.apply(this, arguments);}var _default = { processTypeformV4: processTypeformV4 // testUploadItemImageToS3
