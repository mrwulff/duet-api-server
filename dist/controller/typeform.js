...
function _processTypeformV() {_processTypeformV = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(req, res) {var answers, formTitle, formQuestions, eventId, language, beneficiaryId, phoneNum, photoUrl, itemName, price, size, store, itemTranslationResult, itemNameEnglish, categoryId, storeId, itemId, _sqlHelpers$insertIte, code, s3PhotoUrl;return regeneratorRuntime.wrap(function _callee$(_context) {while (1) {switch (_context.prev = _context.next) {case 0:console.log("Processing TypeForm (V4)");answers = req.body.form_response.answers;formTitle = req.body.form_response.definition.title;formQuestions = req.body.form_response.definition.fields;eventId = req.body.form_response.eventId; // Check which language was used
            ...
            if (!(answers.length >= 8)) {_context.next = 48;break;}beneficiaryId = answers[0].text;phoneNum = answers[1].phone_number;photoUrl = answers[2].file_url;itemName = answers[4].choice.label; // replace "," with "."; remove non-numeric characters
            ...
            _context.next = 18;return _sqlHelpers["default"].getItemNameTranslation(language, itemName);case 18:itemTranslationResult = _context.sent;if (!itemTranslationResult) {console.log("Invalid item name! Table: name_" + language + "; itemName: " + itemName);res.status(500).send();}itemNameEnglish = itemTranslationResult.name_english;categoryId = itemTranslationResult.category_id;_context.next = 24;return _sqlHelpers["default"].getStoreIdFromName(store);case 24:storeId = _context.sent;_context.prev = 25;_context.next = 28;return _sqlHelpers["default"].insertItemFromTypeform((_sqlHelpers$insertIte = { itemNameEnglish: itemNameEnglish, size: size, price: price, beneficiaryId: beneficiaryId }, _defineProperty(_sqlHelpers$insertIte, "beneficiaryId", beneficiaryId), _defineProperty(_sqlHelpers$insertIte, "categoryId", categoryId), _defineProperty(_sqlHelpers$insertIte, "storeId", storeId), _defineProperty(_sqlHelpers$insertIte, "photoUrl", photoUrl), _defineProperty(_sqlHelpers$insertIte, "in_notification", 1), _sqlHelpers$insertIte));case 28:itemId = _context.sent;_context.next = 35;break;case 31:_context.prev = 31;_context.t0 = _context["catch"](25); // Sendgrid Error message (email)
            _sendgridHelpers["default"].sendTypeformErrorEmail({ formTitle: formTitle, eventId: eventId, err: _context.t0 });res.status(500).send();case 35: // get code for item
            code = _itemHelpers["default"].generatePickupCode(itemId);_context.next = 38;return _sqlHelpers["default"].updateItemPickupCode(itemId, code);case 38:_context.next = 40;return _s3Helpers["default"].uploadItemImageToS3(itemId, photoUrl);case 40:s3PhotoUrl = _context.sent;_context.next = 43;return _sqlHelpers["default"].updateItemPhotoLink(itemId, s3PhotoUrl);case 43:_context.next = 45;return _sqlHelpers["default"].setSingleStoreNotificationFlag(storeId);case 45:res.status(200).send();_context.next = 50;break;case 48:console.log("Error! Invalid number of answers.");res.status(502).send();case 50:case "end":return _context.stop();}}}, _callee, null, [[25, 31]]);}));return _processTypeformV.apply(this, arguments);}var _default = { processTypeformV4: processTypeformV4, testUploadItemImageToS3: testUploadItemImageToS3 };exports["default"] = _default;